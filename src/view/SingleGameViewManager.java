...
public class SingleGameViewManager {
	...
	private final static String BACKGROUND_PATH="view/colored_castle.png";
	...
	public void setisWKeyPressed(boolean value) {
		this.isWKeyPressed=value;
	}
	public void setisAKeyPressed(boolean value) {
		this.isAKeyPressed=value;
	}
	public void setisSKeyPressed(boolean value) {
		this.isSKeyPressed=value;
	}
	
	public void setisDKeyPressed(boolean value) {
		this.isDKeyPressed=value;
	}
	...
	private void createSingleGameLoop(TANK choosenTank) {
	...
	public void createNewSingleGame(Stage menuStage, TANK choosenTank) {
		...
		createSingleGameLoop(choosenTank);
		gameStage.show();
	}
	...
	private void moveEnemyTank(TANK choosenTank) {
		String Easy = "model/resources/tankchooser/tanks_tankDesert1.png";
		String Normal = "model/resources/tankchooser/tanks_tankGreen1.png";
		String Hard="model/resources/tankchooser/tanks_tankGrey1.png";
		///EASY
		if(choosenTank.getUrl() == Easy) {
			//does not move
		}
		///NORMAL
		if(choosenTank.getUrl() == Normal) {
			
			if(enemyTank.getLayoutX()<1910-2*TANK_RADIUS && enemyTank.getLayoutX()>900) {
			double myMove = randomPositionGenerator.nextInt(2);
			enemyTank.setLayoutX(enemyTank.getLayoutX()-myMove); 
			gamePane.getChildren().remove(enemyShooterS[0]);
			enemyShooterS[0].setLayoutX(enemyTank.getLayoutX()-40-myMove); 	
			gamePane.getChildren().add(enemyShooterS[0]);
			}
			
			if(enemyTank.getLayoutX()==900) {
				for(int i=0; i<200; i++) {
				double myMove = -randomPositionGenerator.nextInt(3);
				enemyTank.setLayoutX(enemyTank.getLayoutX()-myMove); 
				gamePane.getChildren().remove(enemyShooterS[0]);
				enemyShooterS[0].setLayoutX(enemyTank.getLayoutX()-40+myMove); 	
				gamePane.getChildren().add(enemyShooterS[0]);
		...
		if(choosenTank.getUrl() == Hard) {
			gamePane.getChildren().remove(enemyShooterS[0]);
			double myInfo = playerTank.getLayoutX();
			enemyShooterS[0].setLayoutX(myInfo+85); 
			gamePane.getChildren().add(enemyShooterS[0]);	
				removeEnemyBullet();
				timeIsOut();
				checkIfEnemyShot();
	...
	private void removeEnemyLife(int remove) {
